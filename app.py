...
varification = st.empty()
ans = varification.text_input("请输入验证码")
if st.button("验证"):
    if ans == hashlib.sha256(st.secrets.api_key.encode("utf-8")).hexdigest():
        st.balloons()
        with varification:
            ########################### 一些样式 ###########################
            ...
            if "base_url" not in st.session_state:
                st.session_state["base_url"],st.session_state["model"],st.session_state["temperature"],st.session_state["data_path"],st.session_state["sys_content"],st.session_state["max_tokens"] = get_config(st.session_state["config_path"])
                ...
                with st.container():
                    with st.expander("Settings"):
                        ...
                        if st.button('Save'):
                            ...
                            flag = change_config(st.session_state["config_path"],st.session_state["base_url"],st.session_state["model"],st.session_state["temperature"])
                            if flag:
                                st.balloons()
        ...
        varification.write("Wrong Answer")
